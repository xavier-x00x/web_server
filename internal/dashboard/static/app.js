...
// Removed updateNginxPort since we removed the edit port button
...
function renderSites(sites) {
    if (!sites || sites.length === 0) {
        dom.sitesContainer.innerHTML = `<div class="text-slate-500 text-sm">No sites configured</div>`;
        return;
    }

    const html = sites.map((site, idx) => `
        <div class="flex items-center justify-between mb-3 last:mb-0 bg-slate-800/30 p-2 rounded-xl border border-white/5">
            <div class="flex items-center gap-3 min-w-0">
                <div class="w-8 h-8 rounded-lg bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-xs font-bold text-indigo-400 flex-shrink-0">
                    S${idx + 1}
                </div>
                <div class="min-w-0">
                    <p class="text-sm font-bold text-slate-200 truncate">${site.name}</p>
                    <p class="text-[0.65rem] text-slate-500 font-mono truncate" title="Root: ${site.document_root}">Ports: ${site.base_port}-${site.base_port + site.worker_count - 1} | Root: ${site.document_root}</p>
                </div>
            </div>
            <div class="text-right flex-shrink-0 ml-2">
                <p class="text-xs font-bold text-sky-400">Nginx :${site.nginx_port}</p>
                <p class="text-[0.65rem] text-slate-500">${site.worker_count} workers</p>
            </div>
        </div>
    `).join('');
    
    dom.sitesContainer.innerHTML = html;
}

// ---- Render Workers ----
function renderWorkers(workers) {
    if (!workers || workers.length === 0) {
        dom.workersListContainer.innerHTML = `<div class="flex items-center justify-center py-12 text-sm text-slate-500">No workers found</div>`;
        return;
    }
    ...
    for (const [siteName, siteWorkers] of Object.entries(grouped)) {
        html += `
            <div class="mb-6 last:mb-0">
                <h3 class="text-sm font-bold text-slate-300 mb-3 flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full bg-indigo-500"></span>
                    ${siteName}
                    <span class="text-xs font-normal text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full ml-2">${siteWorkers.length} workers</span>
                </h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        `;
        
        html += siteWorkers.map(w => {
    ...
    dom.workersListContainer.innerHTML = html;
}
